...
var Carousel = _React2['default'].createClass({
  ...
  handleAutoplay: function handleAutoplay(){
    ...
    self.autoplayTimer = window.setTimeout(function(){
      ...
      // console.log('autoplay next', self.state.currentSlide + self.props.slidesToScroll, self.props.children.length);
      // console.log('autoplay back', self.state.currentSlide);

      if(self.autoplayCurrentDirection > 0 && self.state.currentSlide + self.props.slidesToScroll >= self.props.children.length)
        self.autoplayCurrentDirection = -1;
      if(self.autoplayCurrentDirection < 0 && self.state.currentSlide <= 0)
        self.autoplayCurrentDirection = 1;

      if(self.autoplayCurrentDirection > 0)
        self.nextSlide();
      if(self.autoplayCurrentDirection < 0)
        self.previousSlide();
  ...
  animateSlide: function animateSlide(easing, duration, endValue) {
    ...
    this.tweenState(this.props.vertical ? 'top' : 'left', {
      ...
      onEnd: function(){
        if(typeof self.props.onSlideChange == 'function')
          self.props.onSlideChange(self.state.currentSlide);
      }
    });
  },
